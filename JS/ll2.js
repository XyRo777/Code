...
document.addEventListener("DOMContentLoaded", function () {
  fetch("question.json")
    ...
    .then(data => {
      ...
      const mediumLevel = linkedListTopic?.difficultyLevels.find(level => level.level === "Medium");

      if (!mediumLevel) throw new Error("Medium level not found in Linked List topic");

      const stored = JSON.parse(localStorage.getItem('linkedlist_medium_questions'));

      if (!stored || stored.length !== mediumLevel.questions.length) {
        localStorage.setItem('linkedlist_medium_questions', JSON.stringify(mediumLevel.questions));
...
function initializeTableFunctionality() {
  document.addEventListener('change', function (e) {
    ...
    originalQuestions[index].completedOn = new Date().toISOString().split('T')[0];
        ...
        localStorage.setItem('linkedlist_medium_questions', JSON.stringify(originalQuestions));
