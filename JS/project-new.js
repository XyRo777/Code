...
document.addEventListener("DOMContentLoaded", function () {
  fetch("question.json")
    ...
    .then(data => {
      ...
      const hardLevel = arrayTopic?.difficultyLevels.find(level => level.level === "Hard");

      if (!hardLevel) throw new Error("Hard level not found in Arrays topic");

      const storedQuestions = JSON.parse(localStorage.getItem('array_hard_questions'));

      if (!storedQuestions || storedQuestions.length !== hardLevel.questions.length) {
        localStorage.setItem('array_hard_questions', JSON.stringify(hardLevel.questions));
        originalQuestions = hardLevel.questions;
      } else {
    ...
    .catch(err => console.error("Error loading hard questions:", err));
});
...
function initializeTableFunctionality() {
  document.addEventListener('change', function (e) {
    ...
    originalQuestions[index].completedOn = new Date().toISOString().split('T')[0];
        ...
        localStorage.setItem('array_hard_questions', JSON.stringify(originalQuestions));
