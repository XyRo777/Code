...
document.addEventListener("DOMContentLoaded", function () {
  fetch("question.json")
    ...
    .then(data => {
      ...
      const easyLevel = graphsTopic?.difficultyLevels.find(level => level.level === "Easy");

      if (!easyLevel) throw new Error("Easy level not found in Graphs topic");

      const stored = JSON.parse(localStorage.getItem('graphs_easy_questions'));

      if (!stored || stored.length !== easyLevel.questions.length) {
        localStorage.setItem('graphs_easy_questions', JSON.stringify(easyLevel.questions));
...
function initializeTableFunctionality() {
  document.addEventListener('change', function (e) {
    ...
    originalQuestions[index].completedOn = new Date().toISOString().split('T')[0];
        ...
        localStorage.setItem('graphs_easy_questions', JSON.stringify(originalQuestions));
