...
document.addEventListener("DOMContentLoaded", function () {
  fetch("question.json")
    ...
    .then(data => {
      const arrayTopic = data.topics.find(topic => topic.name === "Arrays");
      const easyLevel = arrayTopic?.difficultyLevels.find(level => level.level === "Easy");

      if (!easyLevel) throw new Error("Easy level not found in Arrays topic");

      const storedQuestions = JSON.parse(localStorage.getItem('array_easy_questions'));

      if (!storedQuestions || storedQuestions.length !== easyLevel.questions.length) {
        localStorage.setItem('array_easy_questions', JSON.stringify(easyLevel.questions));
        originalQuestions = easyLevel.questions;
      } else {
        originalQuestions = storedQuestions;
      }
...
function initializeTableFunctionality() {
  document.addEventListener('change', function (e) {
    ...
    originalQuestions[index].completedOn = new Date().toISOString().split('T')[0];
        ...
        localStorage.setItem('array_easy_questions', JSON.stringify(originalQuestions));
...
const stored = JSON.parse(localStorage.getItem('array_easy_questions')) || [];

const completedCount = stored.filter(q => q.status === 'Completed').length;

console.log(`Completed: ${completedCount} / ${stored.length}`);
...
function updateCompletedCount() {
  ...
  document.getElementById('completedCount').textContent =
    completed + ' out of ' + stored.length + ' programs completed';

  localStorage.setItem('completed_array_easy_count', completed.toString());
  localStorage.setItem('total_array_easy_count', stored.length.toString());
}
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  if (loggedInUser) {
    ...
    if (userImage) {
      userImage.src = loggedInUser.image || 'default.jpg';  // assuming 'image' key stores DP
    }
    if (userName) {
      userName.textContent = loggedInUser.fullname || 'Guest User'; // assuming 'fullname' key stores name
    }
  } else {
...
// Prevent accessing page via back after logout
