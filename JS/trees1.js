...
document.addEventListener("DOMContentLoaded", function () {
  fetch("question.json")
    ...
    .then(data => {
      ...
      const easyLevel = treesTopic?.difficultyLevels.find(level => level.level === "Easy");

      if (!easyLevel) throw new Error("Easy level not found in Trees topic");

      const stored = JSON.parse(localStorage.getItem('trees_easy_questions'));

      if (!stored || stored.length !== easyLevel.questions.length) {
        localStorage.setItem('trees_easy_questions', JSON.stringify(easyLevel.questions));
...
function initializeTableFunctionality() {
  document.addEventListener('change', function (e) {
    ...
    originalQuestions[index].completedOn = new Date().toISOString().split('T')[0];
        ...
        localStorage.setItem('trees_easy_questions', JSON.stringify(originalQuestions));
