...
document.addEventListener("DOMContentLoaded", function () {
  fetch("question.json")
    ...
    .then(data => {
      ...
      const hardLevel = linkedListTopic?.difficultyLevels.find(level => level.level === "Hard");

      if (!hardLevel) throw new Error("Hard level not found in Linked List topic");

      const stored = JSON.parse(localStorage.getItem('linkedlist_hard_questions'));

      if (!stored || stored.length !== hardLevel.questions.length) {
        localStorage.setItem('linkedlist_hard_questions', JSON.stringify(hardLevel.questions));
...
function initializeTableFunctionality() {
  document.addEventListener('change', function (e) {
    ...
    originalQuestions[index].completedOn = new Date().toISOString().split('T')[0];
        ...
        localStorage.setItem('linkedlist_hard_questions', JSON.stringify(originalQuestions));
