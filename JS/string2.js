...
document.addEventListener("DOMContentLoaded", function () {
  fetch("question.json")
    ...
    .then(data => {
      ...
      const mediumLevel = stringTopic?.difficultyLevels.find(level => level.level === "Medium");

      if (!mediumLevel) throw new Error("Medium level not found in Strings topic");

      const storedQuestions = JSON.parse(localStorage.getItem('string_medium_questions'));

      if (!storedQuestions || storedQuestions.length !== mediumLevel.questions.length) {
        localStorage.setItem('string_medium_questions', JSON.stringify(mediumLevel.questions));
...
function initializeTableFunctionality() {
  document.addEventListener('change', function (e) {
    ...
    originalQuestions[index].completedOn = new Date().toISOString().split('T')[0];
        ...
        localStorage.setItem('string_medium_questions', JSON.stringify(originalQuestions));
