...
document.addEventListener("DOMContentLoaded", function () {
  fetch("question.json")
    ...
    .then(data => {
      ...
      const mediumLevel = arrayTopic?.difficultyLevels.find(level => level.level === "Medium");

      if (!mediumLevel) throw new Error("Medium level not found in Arrays topic");

      const storedQuestions = JSON.parse(localStorage.getItem('array_medium_questions'));

      if (!storedQuestions || storedQuestions.length !== mediumLevel.questions.length) {
        localStorage.setItem('array_medium_questions', JSON.stringify(mediumLevel.questions));
...
function initializeTableFunctionality() {
  document.addEventListener('change', function (e) {
    ...
    originalQuestions[index].completedOn = new Date().toISOString().split('T')[0];
        ...
        localStorage.setItem('array_medium_questions', JSON.stringify(originalQuestions));
