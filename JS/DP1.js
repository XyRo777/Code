...
document.addEventListener("DOMContentLoaded", function () {
  fetch("question.json")
    ...
    .then(data => {
      const dpTopic = data.topics.find(topic => topic.name === "Dynamic Programming");
      const easyLevel = dpTopic?.difficultyLevels.find(level => level.level === "Easy");

      if (!easyLevel) throw new Error("Easy level not found in Dynamic Programming topic");

      const stored = JSON.parse(localStorage.getItem('dp_easy_questions'));

      if (!stored || stored.length !== easyLevel.questions.length) {
        localStorage.setItem('dp_easy_questions', JSON.stringify(easyLevel.questions));
    ...
    .catch(err => console.error("Error loading easy DP questions:", err));
});
  ...
  document.addEventListener('change', function (e) {
    if (e.target.classList.contains('status-select')) {
      ...
      if (!isNaN(index) && originalQuestions[index]) {
        ...
        localStorage.setItem('dp_easy_questions', JSON.stringify(originalQuestions));
