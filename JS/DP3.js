...
document.addEventListener("DOMContentLoaded", function () {
  fetch("question.json")
    ...
    .then(data => {
      ...
      const hardLevel = dpTopic?.difficultyLevels.find(level => level.level === "Hard");

      if (!hardLevel) throw new Error("Hard level not found in Dynamic Programming topic");

      const stored = JSON.parse(localStorage.getItem('dp_hard_questions'));

      if (!stored || stored.length !== hardLevel.questions.length) {
        localStorage.setItem('dp_hard_questions', JSON.stringify(hardLevel.questions));
    ...
    .catch(err => console.error("Error loading hard DP questions:", err));
});
  ...
  document.addEventListener('change', function (e) {
    if (e.target.classList.contains('status-select')) {
      ...
      if (!isNaN(index) && originalQuestions[index]) {
        ...
        localStorage.setItem('dp_hard_questions', JSON.stringify(originalQuestions));
