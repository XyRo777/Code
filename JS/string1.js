...
document.addEventListener("DOMContentLoaded", function () {
  fetch("question.json")
    ...
    .then(data => {
      const stringTopic = data.topics.find(topic => topic.name === "Strings");
      const easyLevel = stringTopic?.difficultyLevels.find(level => level.level === "Easy");

      if (!easyLevel) throw new Error("Easy level not found in Strings topic");

      const storedQuestions = JSON.parse(localStorage.getItem('string_easy_questions'));

      if (!storedQuestions || storedQuestions.length !== easyLevel.questions.length) {
        localStorage.setItem('string_easy_questions', JSON.stringify(easyLevel.questions));
    ...
    .catch(err => console.error("Error loading easy questions:", err));
});
...
function initializeTableFunctionality() {
  document.addEventListener('change', function (e) {
    ...
    originalQuestions[index].completedOn = new Date().toISOString().split('T')[0];
        ...
        localStorage.setItem('string_easy_questions', JSON.stringify(originalQuestions));
