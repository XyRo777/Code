...
document.addEventListener("DOMContentLoaded", function () {
  fetch("question.json")
    ...
    .then(data => {
      ...
      const mediumLevel = dpTopic?.difficultyLevels.find(level => level.level === "Medium");

      if (!mediumLevel) throw new Error("Medium level not found in Dynamic Programming topic");

      const stored = JSON.parse(localStorage.getItem('dp_medium_questions'));

      if (!stored || stored.length !== mediumLevel.questions.length) {
        localStorage.setItem('dp_medium_questions', JSON.stringify(mediumLevel.questions));
    ...
    .catch(err => console.error("Error loading medium DP questions:", err));
});
  ...
  document.addEventListener('change', function (e) {
    if (e.target.classList.contains('status-select')) {
      ...
      if (!isNaN(index) && originalQuestions[index]) {
        ...
        localStorage.setItem('dp_medium_questions', JSON.stringify(originalQuestions));
