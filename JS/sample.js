  ...
  document.addEventListener("DOMContentLoaded", function () {
    fetch("/json/question.json")
      ...
      .then(data => {
        ...
        updatePieChart();
        updateWeeklyActivityChart(filteredQuestions);
...
function updateWeeklyActivityChart(filteredQuestions) {
  const dateCounts = {};
  const today = new Date();
  const last7Days = getLastNDaysFrom(today, 7);

  last7Days.forEach(date => dateCounts[date] = 0);

  filteredQuestions.forEach(q => {
    if (q.status === "Completed") {
      const completedEntry = loggedInUser.QuestionsCompleted.find(entry => parseInt(entry[0]) === q.id);
      if (completedEntry) {
        const [day, month, year] = completedEntry[1].split(" ")[0].split("-");
  ...
  const chartLabels = last7Days.map(date => getDayShort(date));
  ...
  if (window.activityChart) {
    window.activityChart.destroy(); // Reuse canvas
  }
