...
document.addEventListener("DOMContentLoaded", function () {
  fetch("question.json")
    ...
    .then(data => {
      const linkedListTopic = data.topics.find(topic => topic.name === "Linked List");
      const easyLevel = linkedListTopic?.difficultyLevels.find(level => level.level === "Easy");

      if (!easyLevel) throw new Error("Easy level not found in Linked List topic");

      const stored = JSON.parse(localStorage.getItem('linkedlist_easy_questions'));

      if (!stored || stored.length !== easyLevel.questions.length) {
        localStorage.setItem('linkedlist_easy_questions', JSON.stringify(easyLevel.questions));
...
function initializeTableFunctionality() {
  document.addEventListener('change', function (e) {
    ...
    originalQuestions[index].completedOn = new Date().toISOString().split('T')[0];
        ...
        localStorage.setItem('linkedlist_easy_questions', JSON.stringify(originalQuestions));
